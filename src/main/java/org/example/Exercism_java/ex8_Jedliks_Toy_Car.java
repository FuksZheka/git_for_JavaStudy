...
*/
public class ex8_Jedliks_Toy_Car {
    int distance = 0;
    int batteryCharge = 100;

    public static ex8_Jedliks_Toy_Car buy() {
        return new ex8_Jedliks_Toy_Car();
    }
    ...
    public String batteryDisplay() {
        ...
        return String.format("Battery at %s%%", this.batteryCharge);

    }

    public void drive() {
        if (this.batteryCharge == 0) return;
        this.distance += 20;
        this.batteryCharge -= 1;
    }

    public static void main(String[] args) {
        ex8_Jedliks_Toy_Car Bugatti = ex8_Jedliks_Toy_Car.buy();
        for (int i = 0; i < 100; i++) {
            Bugatti.drive();
        }
        System.out.println(Bugatti.distanceDisplay());
    }
}
